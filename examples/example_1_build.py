...
# Setup and run the stimulation protocol (current clamp step). The delay
# and duration of the step are in ms. The amplitude is the current (in
# nA) applied to the cell; by default this current is added on top of
# the cell's rheobase (change this behaviour with the `add_rheob`
# setting). `tmax` sets the total duration (in ms) of the simulation.
stim = Stim(cell)
stim.set_stim(
    delay=10, duration=150, amplitude=0.2, tmax=200, add_rheob=True
)
stim.run()
